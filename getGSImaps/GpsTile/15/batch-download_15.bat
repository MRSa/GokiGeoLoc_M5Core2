...
REM x の範囲: 28080 ～ 29670
for /L %%x in (28080,1,29670) do (
    REM x の値でサブディレクトリ作成
    if not exist %%x (
        mkdir %%x
    )

    REM y の範囲: 11700 ～ 13420
    for /L %%y in (11700,1,13420) do (
        REM ダウンロード先URLと保存先パス
        set "url=https://cyberjapandata.gsi.go.jp/xyz/std/15/%%x/%%y.png"
        set "outfile=%%x\%%y.png"
